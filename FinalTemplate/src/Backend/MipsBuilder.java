...
import Backend.Instructions.Sra;
import Backend.Instructions.Srl;
...
public class MipsBuilder {
    ...
    public void printToMips(PrintInstruction irInstr) {
        ...
        LinkedList<Value> queue = new LinkedList<>(irInstr.getParams());
        for (int i = 1; i < formatStr.length() - 1; i++) {
            if (formatStr.charAt(i) == '%' && formatStr.charAt(i + 1) == 'd') {
                ...
                String irParam = queue.poll().getName();
    ...
    public void calcToMips(CalcInstruction irInstr) {
        ...
        switch (irInstr.getType()) {
            ...
            case MUL -> {
                if (irInstr.getOp1().isConst()) {
                    int op1 = irInstr.getOp1().getNumber();
                    int pow = isPowerOfTwo(op1);
                    if (pow != -1) {
                        Sll sll = new Sll(targetReg, regOp2, pow);
                        addMipsInstr(MipsInstrType.COMMON, sll);
                        break;
                    }
                } else if (irInstr.getOp2().isConst()) {
                    int op2 = irInstr.getOp2().getNumber();
                    int pow = isPowerOfTwo(op2);
                    if (pow != -1) {
                        Sll sll = new Sll(targetReg, regOp1, pow);
                        addMipsInstr(MipsInstrType.COMMON, sll);
                        break;
                    }
                }
            ...
            case SDIV -> {
                if (irInstr.getOp2().isConst()) {
                    optimizeDiv(targetReg, regOp1, irInstr.getOp2().getNumber());
                    break;
                }
            ...
            case BITAND -> {
                And and = new And(targetReg, regOp1, regOp2);
                addMipsInstr(MipsInstrType.COMMON, and);
            }
        }
    ...
    public void optimizeDiv(int targetReg, int regOp1, int d) {
        ...
        double high = Math.floor((Math.pow(2, N + log) + Math.pow(2, N + log - precision)) / dAbs);
        ...
        long m = (long) high;
        if (dAbs == 1) {
            Move move = new Move(regOp1, targetReg);
            addMipsInstr(MipsInstrType.COMMON, move);
        } else if (dAbs == Math.pow(2, log)) {      // 除2的幂
            Sra sra1 = new Sra(targetReg, regOp1, log - 1);
            addMipsInstr(MipsInstrType.COMMON, sra1);
            Srl srl = new Srl(targetReg, targetReg, N - log);
            addMipsInstr(MipsInstrType.COMMON, srl);
            Addu addu = new Addu(targetReg, targetReg, regOp1);
            addMipsInstr(MipsInstrType.COMMON, addu);
            Sra sra2 = new Sra(targetReg, targetReg, log);
    ...
    public void brToMips(BrInstruction irInstr) {
        if (irInstr.getDest() != null) {        // 无条件跳转 j
            ...
            String cond = irInstr.getCond().getName();
    ...
    public void funcToMips(FuncInstruction irInstr) {
        ...
        Addiu addiu1 = new Addiu(29, 29, -4);     // 栈顶 - 4
        addMipsInstr(MipsInstrType.COMMON, addiu1);
        HashMap<Integer, Integer> regAddrMap = new HashMap<>();
        for (int i = 0; i < irInstr.getParams().size(); i++) {
            String param = irInstr.getParams().get(i).getName();
        ...
        regManager.unmapAll();                                             // 调用函数之前应该是可以解除所有寄存器的
        Jal jal = new Jal(irInstr.getFuncName());
        addMipsInstr(MipsInstrType.COMMON, jal);
        Addiu addiu2 = new Addiu(29, 29, 4);             // 栈顶 + 4
        addMipsInstr(MipsInstrType.COMMON, addiu2);
        Lw restoreRa = new Lw(31, 29, 0);             // 恢复ra
        addMipsInstr(MipsInstrType.COMMON, restoreRa);
        Addiu restoreFp = new Addiu(FP, FP, -fpStackSpace);   // 恢复fp
    ...
    public void cmpToMips(CmpInstruction irInstr) {
        int opReg1 = opToMips(irInstr.getOp1().getName());
        int opReg2 = opToMips(irInstr.getOp2().getName());
        int resultReg = regManager.allocTmpReg();
        // 把比较结果存起来
