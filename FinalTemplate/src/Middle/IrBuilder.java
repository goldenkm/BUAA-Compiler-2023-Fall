...
public class IrBuilder {
    ...
    public IrGlobalVar buildGlobalVar(Def globalDef) {
        IrGlobalVar globalVar = new IrGlobalVar(globalDef, symbolTables.get(currentTableId));
        curSymbolId = findSymbol(globalDef.getIdentifier()).getId();
        return globalVar;
    }

    public IrFunction buildFunction(FuncDef funcDef) {
        IrFunction function = new IrFunction(funcDef.getName(),
        ...
        for (int i = 0; i < funcDef.getFuncFParams().size(); i++) {
            ...
            findSymbol(funcFParam.getIdentifier()).setIrValue(storeInstr);
        ...
        localVarCnt = 0;
        //labelCnt = 0;             // label不能清零，因为mips不允许（llvm可以）
        return function;
    }

    public IrFunction buildMainFunction(MainFuncDef mainFuncDef) {
        IrFunction mainFunc = new IrFunction(
    ...
    public void defToInstr(Def def) {
        ...
        if (def.hasInitialValue()) {
            ...
            switch (def.getType()) {
                case 1 -> {
                    for (int i = 0; i < def.getRvalue().getExps().size(); i++) {
                        ...
                        PtrInstruction gepInstr = new PtrInstruction(
                                InstructionType.PTR, result, pointer, new Value(i));
                    ...
                    for (int i = 0; i < def.getRvalue().getExpArrays().size(); i++) {
                        for (int j = 0; j < def.getRvalue().getExpArrays().get(i).size(); j++) {
                            ...
                            PtrInstruction gepInstr = new PtrInstruction(InstructionType.PTR, result,
                                    pointer, new Value(i), new Value(j));
                ...
                default -> {
                    if (def.getRvalue() instanceof GetIntStmt) {
                        curBasicBlock.addInstruction(getIntStmtToInstr((GetIntStmt) def.getRvalue()));
                        return;
                    }
                    Value rValue = expToInstr(def.getRvalue().getExp());
                    Instruction storeInstr = new MemInstruction(InstructionType.STORE, rValue, allocInstr);
                    curBasicBlock.addInstruction(storeInstr);
                    symbol.setIrValue(storeInstr);
                }
            }
        }
    }
    ...
    public Value addExpToInstr(AddExp addExp) {
        ...
        for (int i = 1; i < addExp.getBases().size(); i++) {
            ...
            if (localVar1.isConst() && localVar2.isConst()) {
                localVar1 = addExp.getOpList().get(i - 1).equals("+") ?
                        new Value(localVar1.getNumber() + localVar2.getNumber()) :
                        new Value(localVar1.getNumber() - localVar2.getNumber());
                continue;
            }
            isConst = false;
            Instruction newInstr = switch (addExp.getOpList().get(i - 1)) {
                case "+" -> new CalcInstruction(InstructionType.ADD,
                        new Value("%local_var_" + localVarCnt), localVar1, localVar2);
                case "-" -> new CalcInstruction(InstructionType.SUB,
                        new Value("%local_var_" + localVarCnt), localVar1, localVar2);
                default -> null;
            };
        ...
        return addExp.getBases().size() == 1 || isConst ? localVar1 : curBasicBlock.getLastInstruction();
    }
    ...
    public Value mulExpToInstr(MulExp mulExp) {
        ...
        for (int i = 1; i < mulExp.getBases().size(); i++) {
            ...
            if (localVar1.isConst() && localVar2.isConst()) {
                localVar1 = switch (mulExp.getOpList().get(i - 1)) {
                    case "/" -> new Value(localVar1.getNumber() / localVar2.getNumber());
                    case "%" -> new Value(localVar1.getNumber() % localVar2.getNumber());
                    case "bitand" -> new Value(localVar1.getNumber() & localVar2.getNumber());
                    default -> new Value(localVar1.getNumber() * localVar2.getNumber());
                };
                continue;
            }
            isConst = false;
            Instruction newInstr = switch (mulExp.getOpList().get(i - 1)) {
                ...
                case "%" -> new CalcInstruction(InstructionType.SREM,
                        new Value("%local_var_" + localVarCnt), localVar1, localVar2);
                case "bitand" -> new CalcInstruction(InstructionType.BITAND,
                        new Value("%local_var_" + localVarCnt), localVar1, localVar2);
                default -> null;
            };
            curBasicBlock.addInstruction(newInstr);
        ...
        return mulExp.getBases().size() == 1 || isConst ? localVar1 : curBasicBlock.getLastInstruction();
    }

    public Value unaryExpToInstr(UnaryExp unaryExp) {       // 需要分情况讨论
        if (unaryExp.getOp().equals("-")) {     // 带负号的unaryExp
            Value op2 = unaryExpToInstr((UnaryExp) unaryExp.getUnaryBase());
            if (op2.isConst()) {
                return new Value(-op2.getNumber());
            }
            CalcInstruction mulInstr = new CalcInstruction(InstructionType.SUB,
        ...
        if (isNumber(name)) {           // 常量或数字，直接返回
            return new Value(Integer.parseInt(name));
        }
        // LValue
            ...
            case 1 -> {         // 一维数组，二维数组加一个[]
                switch (symbol.getType()) {
                    case ARRAY2 -> {        // 二维数组加一个[]
                        ...
                        Value index1 = dealVarDim(exp1);
                        PtrInstruction gepInstr = new PtrInstruction(InstructionType.PTR,
                                result, pointer, index1, new Value(0));
                        curBasicBlock.addInstruction(gepInstr);
                        return gepInstr;
                    }
                    case ARRAY1 -> {
    ...
    public Value funcExpToInstr(FuncExp funcExp) {
        ...
        for (Exp funcParam : funcExp.getFuncParams()) {
            ...
            // 函数参数类型就是此时最后一条指令的类型
            callInstr.addParam(value, value.typeToLlvm());
        }
    ...
    private Value lvalueToInstr(Value value, Lvalue lvalue) {
        ...
        switch (symbol.getType()) {     // 左边的值应该只能是普通变量（如果不是我会崩溃）
            ...
            default -> {
                ...
                symbol.setIrValue(storeInstr);      //更新irValue
                return storeInstr;
            }
        }
    }
    ...
    public void lOrExpToInstr(LOrExp lOrExp,
        ...
        for (int i = 0; i < lOrExp.getBases().size(); i++) {
            ...
            CmpInstruction cmpInstr = new CmpInstruction(
                    InstructionType.ICMP, cmpResult, CmpArgs.NE, andExp, new Value("0"));
            ...
            if (i != lOrExp.getBases().size() - 1) {
                brInstruction = new BrInstruction(
                        InstructionType.BR, cmpInstr, trueLabel, falseLabel);
            } else {
                brInstruction = new BrInstruction(
                        InstructionType.BR, cmpInstr, trueLabel, "unknown");
    ...
    private boolean isNumber(String s) {
        for (int i = 0; i < s.length(); i++) {
            if (s.charAt(i) == '-' && i == 0) {     // 只有第一位是0的时候合法
                continue;
            }
    ...
    private Value dealParamArray(Symbol symbol) {     // 处理函数传过来的数组
        ...
        if (isFuncParam) {     // 如果是函数传过来的数组，就需要先
            Value loadResult = new Value("%local_var_" + localVarCnt++);
            if (pointer.getVarType() == 6) {        // 若是二维数组，则需要记录第二维是多少
                loadResult.setCol(symbol.getDim2());
            }
    ...
    private Value dealVarDim(Exp indexExp) {
        return expToInstr(indexExp);
    }
}
