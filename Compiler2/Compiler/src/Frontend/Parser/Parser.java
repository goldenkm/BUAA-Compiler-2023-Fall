...
public class Parser {
    ...
    private void addSymbol(int line, String token, int type, boolean isConst) {
        Symbol symbol = new Symbol(line, currentTableId, token, type, isConst);
    ...
    private void createSymbolTable() {
        ...
        symbolTables.put(currentTableId, new SymbolTable(currentTableId, fatherTableId));
    }
    ...
    private boolean hasParam() {
        if (getCurTokenType().equals("IDENFR") || getCurTokenType().equals("INTCON")
    ...
    private Def parseConstDef() {
        ...
        addSymbol(line, ident, type, true);
    ...
    private Def parseVarDef() {
        ...
        varDef.setType(type);
        addSymbol(line, ident, type, false);
        if (curToken.equals("=")) {
            next();
    ...
    private FuncDef parseFuncDef() {
        ...
        FuncSymbol funcSymbol = new FuncSymbol(line, currentTableId, funcName, 3, false, currentFuncType);
        next();         // read 'ident'
        next();         // read '('
    ...
    private Block parseBlock(ArrayList<FuncFParam> funcFParams) {
        ...
        for (FuncFParam funcFParam : funcFParams) {
            addSymbol(funcFParam.getLine(), funcFParam.getIdentifier(), funcFParam.getType(), false);
        }
        ...
        fatherTableId = symbolTables.get(currentTableId).getFatherId();
        return block;
    }
