...
public class Lexer {
    ...
    private String[] keywords = {"main", "const", "int", "break", "continue",
            "if", "else", "for", "getint", "printf", "return", "void", "repeat", "until"};
    ...
    public void work(int line) {
        while (pos < input.length()) {
            ...
            if (Arrays.asList(whitespace).contains(String.valueOf(c))) {     // 读入空白字符
                pos++;
                continue;
            }
            String token = "";
            ...
            if (Character.isAlphabetic(c) || c == '_') {               //读入字符
                ...
                if (c == '0' && input.charAt(pos + 1) == 'x') {     //0x123
                    token = getHexNum("0x");
                    type = "HEXCON";
                } else if (c == '0' && input.charAt(pos + 1) == 'X') {        // 0X123
                    token = getHexNum("0X");
                    type = "HEXCON";
                } else {
                    token = getNum();
    ...
    public String getHexNum(String prefix) {
        StringBuilder sb = new StringBuilder();
        sb.append(prefix);
        pos += 2;
    ...
    public void initializeMap() {
        ...
        // 去年期中考试内容
        tokenTypeMap.put("repeat", "REPEATTK");
        tokenTypeMap.put("until", "UNTILTK");
        tokenTypeMap.put("HexConst", "HEXCON");
    }
}
